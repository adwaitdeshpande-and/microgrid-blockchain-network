...
docker exec -it cli sh -c "cd /opt/gopath/src && go mod init chaincode && go mod tidy"
...
do
  ...
  docker exec -ti cli sh -c "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH CORE_PEER_ADDRESS=$PEER_ADDR:7051 CORE_PEER_LOCALMSPID=$CORE_PEER_LOCALMSPID CORE_PEER_TLS_ROOTCERT_FILE=$CORE_PEER_TLS_ROOTCERT_FILE peer chaincode install -n $CONTRACT1_ID -v 1.0 -p /opt/gopath/src/chaincode/$CONTRACT1_ID -l golang"
  sleep 3s
  docker exec -ti cli sh -c "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH CORE_PEER_ADDRESS=$PEER_ADDR:7051 CORE_PEER_LOCALMSPID=$CORE_PEER_LOCALMSPID CORE_PEER_TLS_ROOTCERT_FILE=$CORE_PEER_TLS_ROOTCERT_FILE peer chaincode install -n $CONTRACT2_ID -v 1.0 -p /opt/gopath/src/chaincode/$CONTRACT2_ID -l golang"
  sleep 3s
done
